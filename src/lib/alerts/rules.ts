...
async function ruleCashflowNegative(orgId: string): Promise<RuleResult | null> {
  const now = new Date();
...
async function ruleCostOverBudget(orgId: string): Promise<RuleResult | null> {
  const now = new Date();
  ...
  for (const r of records) {
    const cat = (r.description ?? '').split('/')[0]?.trim() || 'other';
    const mo = monthKey(r.occurredAt);
...
async function ruleGrossMarginLow(orgId: string): Promise<RuleResult | null> {
  const now = new Date();
  const threeMonthsAgo = addMonths(now, -3);
...
async function ruleRevenueStagnant(orgId: string): Promise<RuleResult | null> {
  const now = new Date();
...
export async function runAllRules(organizationId: string): Promise<RuleResult[]> {
