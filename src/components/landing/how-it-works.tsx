          ...
          {steps.map((s, idx) => {
            ...
            return (
              <motion.div
                ...
                className="relative rounded-lg border border-slate-200 bg-[#f8fafc] p-6"
              >
                <span className="absolute right-4 top-4 font-mono text-xs font-medium text-slate-400">
                  0{idx + 1}
                </span>
                ...
                <p className="mt-1 text-sm text-slate-600">{t(`steps.${s.id}.description`)}</p>
              </motion.div>
            );
          })}
        </div>
