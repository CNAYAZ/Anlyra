...
import { revenueSchema, type RevenueInput } from '@/lib/validators';
import { FormField } from './form-field';
import { postJson } from '@/lib/fetcher';
import { useToast } from '@/components/ui/toaster';

const CATEGORIES = ['sales', 'subscription', 'services', 'licensing', 'other'] as const;
const CURRENCIES = ['EUR', 'USD', 'GBP'];

type Props = { onSaved?: () => void };

export function RevenueForm({ onSaved }: Props) {
  const t = useTranslations('manual');
  const tc = useTranslations('manual.revenue.categories');
  ...
  const submit = async (data: RevenueInput, addAnother: boolean) => {
    try {
      await postJson('/api/data/manual', { type: 'REVENUE', data });
